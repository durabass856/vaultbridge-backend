...
def get_health():
    sql = """SELECT h.*, s.startup_name FROM startup_health_score h
             JOIN startup s ON h.startup_id=s.startup_id ORDER BY h.score_date DESC"""
    data, err = query(sql)
...
def create_health():
    b = request.get_json()
    sql = """INSERT INTO startup_health_score
             (startup_id,score_date,financial_score,team_score,product_score,
              market_score,overall_score,risk_flag,scored_by,notes)
             VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
    data, err = query(sql,(b.get("startup_id"),b.get("score_date"),b.get("financial_score"),
                           b.get("team_score"),b.get("product_score"),b.get("market_score"),
                           b.get("overall_score"),b.get("risk_flag","Green"),
                           b.get("scored_by"),b.get("notes")),fetch="none")
