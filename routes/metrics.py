...
def get_metrics():
    sql = """SELECT m.*, s.startup_name FROM startup_operational_metrics m
             JOIN startup s ON m.startup_id=s.startup_id ORDER BY m.snapshot_date DESC"""
    data, err = query(sql)
...
def create_metric():
    ...
    sql = """INSERT INTO startup_operational_metrics
    ...
    data, err = query(sql,(b["startup_id"],b.get("snapshot_date"),b.get("monthly_revenue_usd"),
                           b.get("monthly_burn_usd"),b.get("runway_months"),b.get("gross_margin_pct"),
                           b.get("customer_count"),b.get("mrr_usd"),b.get("churn_rate_pct"),
                           b.get("nps_score"),b.get("source","Self-reported")),fetch="none")
